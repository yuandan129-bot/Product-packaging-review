...
const STEPS: StepDef[] = [
  { id: 'extract', label: 'OCR 文字提取', desc: 'Tesseract 提取文字' },
...
export default function AnalyzePage() {
  ...
  /* ── OCR 文字提取（浏览器端 Tesseract.js）── */
  const runOCR = async () => {
    if (!uploadedImage) return
    setIsOcrRunning(true)
    setActiveStep('extract')
    addLog('system', '── OCR extraction started ──')
    addLog('vision', 'loading Tesseract.js engine...')

    try {
      const { createWorker } = await import('tesseract.js')
      addLog('vision', 'Tesseract worker created · loading chi_sim...')

      const worker = await createWorker('chi_sim+eng')
      ocrWorkerRef.current = worker

      addLog('vision', 'OCR engine ready · recognizing text...')
      ...
      if (!text.trim()) {
        addLog('error', '✗ OCR 未识别到文字，请确认图片清晰度')
        setIsOcrRunning(false)
        setActiveStep('extract')
        return
      }

      addLog('success', `✓ OCR complete · ${text.length} chars extracted`)
      setOcrText(text)
          ...
          <div className={styles.splitSticky}>
            ...
            <div className={styles.ocrSection}>
              ...
              <textarea
                ...
                placeholder={isImageMode ? '点击下方"OCR 提取文字"开始识别...' : '已粘贴的文字内容...'}
                rows={14}
